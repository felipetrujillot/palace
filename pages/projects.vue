  ...
  <div class="container-sm space-y-4">
    <div class="flex justify-between">
      <div>
        ...
        <h1 class="text-3xl font-bold text-primary">Propuestas</h1>
        <p class="text-muted-foreground">Listado de tus proyectos</p>
      </div>
      <div>
        <Button @click.prevent="$router.push('/project/new')">Nuevo Proyecto</Button>
      </div>
    </div>
    ...
    <template v-if="!pending && projects">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 align-stretch gap-4">
        <Card @click.prevent="$router.push(`/project/${p.id_project}`)" class="cursor-pointer" v-for="p in projects"
          :key="p.id_project">
          ...
          <Button class="w-full border-primary text-primary hover:text-primary" variant="outline">Ver Propuesta</Button>
        </Card>
      </div>
    </template>
  </div>
